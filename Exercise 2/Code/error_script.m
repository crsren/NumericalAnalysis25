...
for ind = 4:9 % choose these carefully
 h_v = 10^(-ind); % set stepsize
 [tout, qout] = RK2(qcFunc, t0, qc0, h_v, a, tf); %Numerical method
 ...
 errorMax = max( abs(qcExact - qout) ); % calculate maximum error over range of x
 plot(log(h_v), log(errorMax), 'b*'); % log/log plot stepsize vs error
 hold on; % for next value to be plotted
end
ylabel("log(max absolute error)");
xlabel("log(time interval)");
title("Error Analysis");
figure;

<<<<<<< HEAD:Exercise 2/Code/error_scrpit.txt
...
>>>>>>> 023df33fe4991fe7fb230479bee6a52e0357d55e:Exercise 2/Code/error_script.m
...
<<<<<<< HEAD:Exercise 2/Code/error_scrpit.txt
errorAbs = abs(error);
loglog(tout, errorAbs); %now plot the error using a log-log plot to see that mag.
ylabel("log(absolute error)");
xlabel("log(t)");
title("Approximation Error in log scale");
=======
